...
export async function getSourcePageLayout(book: StoredBook, page: number): Promise<SourcePageLayout> {
  ...
  try { return JSON.parse(await readFile(cachePath, "utf8")) as SourcePageLayout; }
  ...
  const job = limiter.run(2, async () => {
    ...
    if (!layout?.words.length) {
      ...
      try {
        ...
        const result = await run("tesseract", [input, "stdout", "-l", "eng+chi_sim+chi_tra+jpn", "tsv"], { timeout: 60_000, maxBuffer: 8 * 1024 * 1024, env: { ...process.env, OMP_THREAD_LIMIT: "1" } });
