...
export const UI_MESSAGES = {
  "zh-CN": {
    translationUsageLabel: "本页翻译 API 用量",
    translationTotalLabel: "总 tokens",
    translationInputHelp: "输入 tokens（含缓存输入）",
    translationOutputHelp: "API 报告的输出 tokens（含推理）",
    translationTpsHelp: "输出 tokens / 首个输出至响应完成的秒数。不含排队和排版耗时；非流式响应或缺少计时数据时显示 —。",
    translationCacheLabel: "缓存",
    translationCacheHelp: (cached: string) => `缓存输入 ${cached} tokens；命中率 = 缓存输入 / 总输入。API 未报告时显示 —。`,
    translationCostLabel: "费用",
    translationCostHelp: "按翻译时配置的单价估算，已计入缓存输入折扣。重新打开保留原费用。",
    pricingLabel: "翻译费用 · 币种",
    pricingHelp: "单价单位：所选币种 / 百万 tokens。填写输入和输出单价后，新翻译会显示估算费用。缓存单价留空时使用普通输入单价；单独设置缓存价但 API 未报告缓存用量时不估算。切换模型或 API 地址后请重新填写。",
    pricingInput: "输入单价 / 百万 tokens",
    pricingOutput: "输出单价 / 百万 tokens",
    pricingCachedInput: "缓存输入单价 / 百万 tokens（可选）",
    pricingNotSet: "未设置",
    pricingSchedule: "计价方式",
    pricingFixed: "固定单价",
    pricingDeepseek: "DeepSeek 峰谷自动切换",
    pricingDeepseekHelp: "下方填写谷时单价。北京时间周一至周五 09:00–12:00、14:00–18:00 按 2 倍计价，其余时段按谷价。按每次 API 请求发出时间估算；跨时段请求以服务商账单为准。",
    pricingPeriods: { peak: "峰时", offPeak: "谷时", mixed: "混合时段" },
  ...
  "en-US": {
    ...
    translationUsageLabel: "Translation API usage for this page",
    translationTotalLabel: "Total tokens",
    translationInputHelp: "Input tokens, including cached input",
    translationOutputHelp: "Provider-reported output tokens, including reasoning",
    translationTpsHelp: "Output tokens / seconds from first output to response completion. Excludes queuing and layout work; — for non-streamed responses or missing timing.",
    translationCacheLabel: "Cache",
    translationCacheHelp: (cached: string) => `Cached input: ${cached} tokens; hit rate = cached input / total input. — when unreported.`,
    translationCostLabel: "Cost",
    translationCostHelp: "Estimated using the prices configured when translating, with cached input pricing applied. Reopening preserves the original cost.",
    pricingLabel: "Translation cost · Currency",
    pricingHelp: "Prices are in the selected currency per million tokens. Set input and output prices to estimate costs for new translations. A blank cache price uses the regular input price; a separate cache price requires reported cache usage. Re-enter prices after changing the model or API endpoint.",
    pricingInput: "Input price / million tokens",
    pricingOutput: "Output price / million tokens",
    pricingCachedInput: "Cached input price / million tokens (optional)",
    pricingNotSet: "Not set",
    pricingSchedule: "Pricing schedule",
    pricingFixed: "Fixed prices",
    pricingDeepseek: "DeepSeek peak / off-peak",
    pricingDeepseekHelp: "Enter off-peak prices below. Prices double Monday–Friday, 09:00–12:00 and 14:00–18:00 Beijing time (UTC+8). All other hours use off-peak prices. Estimates use each API request's send time; the provider's bill determines charges for requests crossing a boundary.",
    pricingPeriods: { peak: "Peak", offPeak: "Off-peak", mixed: "Mixed periods" },
