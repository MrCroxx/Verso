...
test('reader zoom preserves the pointer anchor and page layout', {
  timeout: 90_000,
  ...
  const directory = await mkdtemp(path.join(tmpdir(), 'verso-reader-zoom-'));
  ...
  const child = spawn(electron, ['tests/fixtures/reader-zoom.mjs'], { env, stdio: ['ignore', 'pipe', 'pipe'] });
